...
{
    //int input == 0; you can never assign something with ==
	int input = 0; 

    printf("Please input an integer: ");
    scanf("%d", &input);

    //if (is_odd(input) = 1) you cannot compare values with a single =
	if (is_odd(input) == 1)
    {
        printf("%d is an odd number!\n", input);
    }

    //if (is_even(input) = 1) you cannot compare values with a single =
	if (is_even(input) == 1)
    {
